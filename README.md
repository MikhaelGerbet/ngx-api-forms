...
This way, field-level errors are parsed once at the HTTP layer. Components can then read from the error store, or still use `bridge.applyApiErrors()` for form-specific handling.
...
```typescript
// Option 1: Custom constraintMap to override specific mappings
const bridge = createFormBridge(form, {
  preset: laravelPreset(),
  constraintMap: { 'mon_erreur_custom': 'required' },
});

// Option 2: Use catchAll to apply all unmatched errors as 'generic'
const bridge = createFormBridge(form, {
  preset: laravelPreset(),
  catchAll: true,
});
